...
int main(void)
{
  ...
  for(int i = 0; i < 6; i++){
    write_to_address(startAddress + i, bitVals[i]);
  }
  ...
  uint32_t adcVal = pollADC();
	// TODO: Get CRR
  uint32_t brightness = ADCtoCCR(adcVal);
  
  // Update PWM value
	__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_3, brightness);
...
void TIM16_IRQHandler(void)
{
  ...
  uint8_t number = read_from_address(currentAddress);
  char stringToWrite[3]; // Ensure this buffer is large enough for the largest expected number
  sprintf(stringToWrite, "%d", number);
  lcd_command(0xC0);
  for (int i = 0; i < 3; i++) { // Assuming a 16x2 LCD
    lcd_putchar(' '); // Write a space character
  }
  if(number != bitVals[currentAddress]){
    lcd_command(0xC0);
    lcd_putstring("SPI ERROR!");
  }
  else{
    writeLCD(stringToWrite);
    // TODO: Change LED pattern; output 0x01 if the read SPI data is incorrect
  }
  currentAddress ++;
  currentAddress =  currentAddress % 6;
}

// TODO: Complete the writeLCD function
void writeLCD(char *char_in){
  lcd_command(0xC0); // 0xC0 is the command to set the cursor to the beginning of Line 2
  // Write each character of the string to the LCD
  while (*char_in) {
      lcd_putchar(*char_in); // Write each character to LCD
      char_in++;
  }
  delay(3000);
}
...
uint32_t ADCtoCCR(uint32_t adc_val){
  ...
  uint32_t brightness = (adc_val * 47999) / 4096;
	//return val;
  return brightness;
}
